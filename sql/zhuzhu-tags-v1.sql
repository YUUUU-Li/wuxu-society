...
-- 执行方式: Cloudflare D1 控制台(数据库页 -> Console)整段粘贴本文件内容
--   或命令行: npx wrangler d1 execute wuxu-database --remote --file=sql/zhuzhu-tags-v1.sql
...
--   或在作品页众注区「编委 -> 初始化/更新标签词表」点一次(与第一节等价, 幂等)。
--   scripts/test-tags-sql.js 校验本文件的词表与归并表跟 JSON 一致, 防止两份漂移。

-- 两处与早期讨论稿不同、以 docs/标签大纲.md §2 定论为准的取舍:
--   1) 不落 category 列: 分类(cat)与释义(hint)随 tag-outline.js 编译下发, 库里不再存副本,
--      免得两处真相; 下面按四大类书写, 只为便于人读。
--   2) 不建 tag_aliases 吸附表: 同义词不做自动映射, 由编委「合并」动作处理(票数并入目标词)。
--      第二节「历史写法归并」是那 15 条起步标签的一次性清洗, 用的是函数里 cleanup 动作同一张表。

-- ══ 一、受控词表落库(94 词, kind='预设'; 已在库的词原样不动) ══
...
-- 注: 大纲内的词经 /api/tags 投票落库时函数已按词表判定为「预设」, 不会是「候选」,
--     故这里不需要额外的「候选转正」语句; 若库里确有大纲词仍是候选(历史数据),
--     在编委入口点一次「初始化/更新标签词表」即可转正。

-- ══ 二、历史写法归并(一次清洗, 幂等; 与函数 /api/tags 的 cleanup 动作同表) ══
...
-- ══ 四、核对(可选: 粘贴后运行, 看结果) ══
-- SELECT kind, COUNT(*) AS n FROM tags GROUP BY kind;   -- 预期 预设 = 94
-- SELECT COUNT(*) AS votes FROM tag_votes;              -- 预期 = 归并前后票数不变(弃用词除外)
